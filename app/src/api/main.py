...
def build_app():
    ...
    # Denetim/yönetim uçları (`/log`, `/admin/*`, `/contradictions*`)
    # `routers/denetim.py`'ye taşındı — bölmenin 6. ve SON adımı (plan:
    # docs/rapor/api-bolme-plani.md). `app` parametre geçiyor çünkü `/log`
    # günlüğe `app.state.gunluk` test kancası üzerinden erişiyor (3. adımdaki
    # aynı desen).
    app.include_router(denetim.router_kur(
        app, repo,
        campaign_contradictions=_campaign_contradictions,
    ))

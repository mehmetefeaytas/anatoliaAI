...
| 2 | Açık kaynak lisans (Apache-2.0) | §5.10 (s.11), §8 | ✅ | `LICENSE` (kök) · `app/LICENSE` · `app/NOTICE` · `app/docs/model-license-audit.md` · `app/docs/LISANSLAR.md` · `app$ make lisans-kapisi` | İkisi de yerinde. `NOTICE` model ağırlıklarının `base_model` zincirini köke kadar veriyor; Gemma/Llama lisanslı ağırlıklar bilinçli reddedilmiş. ⚠️ Lisans kapısı **CI'da koşmuyor** — `ci.yml`'de lisans işi yok, yalnız yorum satırı var. |
...
| 5 | **README — veri seti indirme bağlantısı** | §9 (s.18) | ❌ | `kök$ grep -n "indirme bağlantısı" README.md` → satır 270: *"(yükleme tamamlandığında buraya eklenecektir)"* | **Şartnamenin açık zorunluluğu ve bugün karşılanmıyor.** Yer tutucu duruyor. Aynı eksik `app/docs/rapor/anatolia-ai-teknik-rapor.md` §E2'de de ❌ olarak kayıtlı. |
| 6 | Veri setinin herkese açık yayını | §9 (s.18) | ❌ | Paketleyici var: `app$ python -m scripts.veri_seti_paketi --gold data/gold/gold.v2.json --out dist/anatolia-ai-veri-seti` (`app/scripts/veri_seti_paketi.py`) | Paket **tek komutla üretilebiliyor** ve provenance kapısından geçiyor (kaynağı doğrulanamayan tek kayıtta çıkış kodu 1). Ama hiçbir yere **yüklenmedi**. Gerekçe kayıtlı: gold seti round1'de genişletiliyor; yarım gold yayımlamak indirilen kopyayı yanlış bırakır. |
| 7 | Veri seti lisansı | §8 | ❌ | `app/docs/model-license-audit.md:308` — kutu **işaretsiz**: "Veri seti lisansı belirtilmiş (CC-BY-4.0)" | Paketleyici pakete `LICENSE` (Apache-2.0, kod+anotasyon) koyuyor ama **veri setine ayrı bir açık lisans (ör. CC-BY-4.0) atanmadı**. Kaynak metinlerin telifi bankalara ait; bu ayrımın lisans metninde yazılı olması gerekiyor. |
...
| 13 | En az haftalık GitHub güncellemesi | §9 (s.18) | 🟠 | `kök$ git tag -l` → `hafta-00` (27 Tem) · `hafta-01` (31 Tem) · `hafta-02` (4 Ağu) · `hafta-03` (5 Ağu) · `hafta-04` (7 Ağu) · `hafta-05` (8 Ağu) | Altı haftalık etiketin altısı da var ve aralıklar haftanın altında. **Ama:** `hafta-05`'ten bu yana **7 gün** geçti ve `hafta-06` etiketi yok. Ayrıca bu klonun commit geçmişi 12–15 Ağustos aralığını kapsıyor (506 commit); **GitHub'a push kadansı yerelden doğrulanamaz**. |
| 14 | `BilisimVadisi2026` etiketi | §9 (s.18) | 🟠 | `README.md:6` — *"Yarışma etiketleri: `BilisimVadisi2026`"* | README'de **beyan** var. Şartnamenin istediği GitHub **depo topic'i** yerel klondan görülemez (topic'ler git nesnesi değildir). Tarayıcıdan teyit edilmeli. |
| 15 | Türkiye Açık Kaynak Platformu etiketi | §9 (s.18) | 🟠 | `README.md:6` | Satır 14 ile aynı gerekçe: beyan var, topic doğrulanamadı. |
| 16 | **On-premise çalışabilirlik** | §5.9 (s.10–11) | 🟠 | `app/docs/OFFLINE-KANIT.md` · `app$ bash scripts/offline_proof.sh` · `app/docker-compose.yml` · `app/Dockerfile.api` | Kanıt **gerçek ve ölçülmüş**: `--network none` içinde 14/14 adım, negatif kontrol (ağ probu 4/4 engellendi) **ve onun pozitif kontrolü** (ağ açıkken 4/4 ulaşıyor), üç imaj `@sha256:` digest pin'li. **Neden ✅ değil — belgenin kendi itirafı (§0-b, §10):** (a) koşum **31 Temmuz**'dan, o gün ağaç kirliydi ve depo 136 commit ilerledi; (b) kanıt **yalnız API konteynerini** kapsıyor, `docker compose up` tam yığını (Postgres, web, vLLM, Ollama) ağsız denenmedi; (c) **imaj derlemesi internet gerektiriyor** — "internetsiz çalışır" iddiası *önceden derlenmiş imajlarla* doğrudur. Bu kapsam sınırı sunumda önce **bizim** söylediğimiz bir cümle olmalı. |
...
| **Teknik İmplementasyon ve Mimari** | %20 | ✅ | `app$ python -m pytest` (**2.649** toplanan · 2.596 geçen) · `.github/workflows/ci.yml` (5 iş: bağımlılıksız testler, bağımlılıklı testler, Postgres/pgvector paritesi, ruff, frontend) · CI adımları: değişmez denetimi · eval regresyon kapısı · gold kanıt zinciri · markdown link denetimi · demo DB tazeliği | Katmanlar bağımsız test edilebilir (`src/scraping` · `preprocessing` · `extraction` · `normalization` · `comparison` · `rag` · `chatbot` · `api` · `db`). Her çıkarılan alan `span_start`/`span_end` taşıyor ve `verify_span()` ile `text[start:end] == raw_value` kendi kendini denetliyor. CI'da eval regresyon kapısı (alan F1 tabanı + halüsinasyon tavanı) var — ölçüm gerilerse build kırılır. ⚠️ Bilinen boşluk: pgvector kurulu ama üretim yolunda kullanılmıyor; lisans kapısı CI'da koşmuyor. |
...
**Ağırlıklı okuma:** kanıtlı ✅ olan iki kriter toplam **%30** (Teknik
İmplementasyon %20 + Yenilikçilik %10). Kalan **%70**'in üçü de 🟠 —
ve en ağırı olan **%30'luk Model Başarısı**, insan çapası olmadan ✅'e
çıkamaz.
...
| # | Kalem | Aksiyon |
|---:|---|---|
| 1 | Veri seti indirme bağlantısı (§9 s.18) | Round1 gold turu biter bitmez `app$ python -m scripts.veri_seti_paketi` çıktısını GitHub Release / Hugging Face Datasets / Zenodo'ya yükle ve URL'yi `README.md:270`'teki yer tutucuyla değiştir. |
| 2 | Veri seti yayını | Yükleme öncesi provenance kapısının çıkış kodunun 0 olduğunu doğrula (tek kaynaksız kayıt paketi geçersiz kılar). |
| 3 | Veri seti lisansı (§8) | Pakete **CC-BY-4.0** `DATA-LICENSE` dosyası ekle; kaynak metin telifinin bankalarda kaldığını aynı dosyada yaz ve `model-license-audit.md:308` kutusunu işaretle. |
...
| 16 | Lisans kapısı otomasyonu | `make lisans-kapisi`'nı `.github/workflows/ci.yml`'e iş olarak ekle — şu an yalnız elle koşuyor. |

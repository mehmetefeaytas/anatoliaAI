...
| 10 | Sunum materyali — **PDF** | §6.4 (s.14) | ✅ | `app/docs/sunum/anatolia-ai-sunum.pdf` — **1,0 MB · 15 sayfa · 1440×810 pt** (oran 1,7778 = tam 16:9). Üretim: `app$ .venv/bin/python docs/sunum/uret-sunum.py` (~5 sn, çevrimdışı; `--sadece pdf\|pptx`) | **Bağımsız doğrulandı (2026-08-16):** `pypdf` ile 15 sayfa; metin katmanı **vektör/seçilebilir** (6.351 karakter çıkarıldı, taranmış görüntü değil); Türkçe diyakritikler metin katmanından sayıldı — `ş 52 · ç 54 · ı 257 · ğ 51 · ü 82 · ö 26 · **â 4**` (â'nın varlığı «k**â**r payı»nın doğru dizildiğini gösterir, mojibake yok). ⚠️ Dosya **henüz commit edilmemiş** (`git status` → `??`); kök `.gitignore:5`'teki `*.pdf` deseni artık istisna ile aşılmış, yani commit edilebilir durumda — teslimden önce **eklenmeli**. |
| 11 | Sunum materyali — **PPTX** | §6.4 (s.14) | ✅ | `app/docs/sunum/anatolia-ai-sunum.pptx` — **1,9 MB · 15 slayt · 20″×11,25″** (her slaytta tam-sayfa 1920×1080 PNG). Aynı üreteç: `docs/sunum/uret-sunum.py` | **Bağımsız doğrulandı (2026-08-16):** `ppt/slides/slideN.xml` sayımı **15**; `zipfile.testzip()` **hatasız**. Üretimde kullanılan `python-pptx` (**MIT**) proje `.venv`'ine **KURULMADI** — ayrı yardımcı ortamda (`~/.cache/anatolia-ai/sunum-venv`) koşuldu, böylece SBOM kirlenmedi; `.venv`'de `python-pptx` yok (ölçüldü). ⚠️ Bu dosya da **henüz commit edilmemiş** (`??`). |
...
| ~~6~~ | ~~Sunum PDF (§6.4)~~ | ✅ **KAPANDI (16 Ağu)** — `docs/sunum/anatolia-ai-sunum.pdf`, **15 sayfa**, 1440×810 pt (16:9), metin katmanı vektör/seçilebilir. Üreteç: `.venv/bin/python docs/sunum/uret-sunum.py`. *(Plandaki «8 slayt» hedefi aşıldı; kaynak HTML `01`–`15` taşıyor — aşağıdaki nota bakınız.)* |
| ~~7~~ | ~~Sunum PPTX (§6.4)~~ | ✅ **KAPANDI (16 Ağu)** — `docs/sunum/anatolia-ai-sunum.pptx`, **15 slayt**, 20″×11,25″. Aynı üreteç. `python-pptx` (MIT) `.venv`'e kurulmadı. |

> **Slayt sayısı: 8 değil, 14 değil — 15.** `sunum-ve-demo-plani.md` §C bir
> **plan** olarak 8 slayt öngörüyordu; üretilen sunum 15 slayttır. Deponun
> geçmişinde «14 slayt» diyen bir commit mesajı da var (`7127045`). Doğrulama
> **sayarak** yapıldı, plana ya da commit mesajına güvenilmedi:
> ```bash
> .venv/bin/python -c "
> import zipfile,re

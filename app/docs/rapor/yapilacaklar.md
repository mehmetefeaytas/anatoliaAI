...
| ~~5.5~~ | LLM çağrısında toplam süre sınırı yok | **kapandı** — duvar-saati sınırı eklendi (`LLM_DEADLINE_CARPANI`, varsayılan soket zaman aşımının 1,5 katı). Ölçülmüş donma: bir koşum `OLLAMA_TIMEOUT=900` ile 18 dk asılı kaldı, çünkü tetiklenecek `recv` yoktu |

...
[![Testler](https://img.shields.io/badge/testler-3578%20ye%C5%9Fil-brightgreen.svg)](tests/)
...
**Test:** **3.632** birim/entegrasyon testi toplanıyor · **3.578 geçiyor** ·
...
# 2026-08-23 (temiz ağaç): 3632 toplandı · 3578 geçti · 54 atlandı · 0 başarısız
```
...
> |---|---:|---:|---:|---:|
> | `unittest` (kanonik — `scripts.test_ozeti`) | **3.632** | **3.578** | 54 | **0** |
> | `pytest` (`pytest tests/ -q`) | **3.632** | **3.578** | 54 | **0** (+1.724 subtest) |
>
...
> `adb9485f`) — yani jürinin ve CI'ın gördüğü sayı. Depoda izlenen dosyaların
> tamamı ve yalnız onlar varken 54 test atlanıyor.
>
> Geliştirici çalışma kopyasında 53 görünür ve fark tek bir testtir:
> `tests/test_xlsx_to_review_csv.py::TestGercekDosyalar` anotatörlerden gelen
> gerçek `.xlsx` dosyalarını okumayı dener. O dosyalar `app/.gitignore:54`
> (`*.xlsx`) ile BİLEREK depo dışında; test de yokluğunda `skipTest` ile
> çekimser kalacak biçimde yazılmış. Yani dosyalara sahip olan makinede test
> KOŞAR — orada geçen sayısı bir artar, atlanan sayısı bir azalır. Temiz
> klonda ATLAR; kanonik sayı bu ikincisidir.
>
> Bu fark 23 Ağu 2026'da ilk kez ölçüldü ve buraya yazıldı: daha önce
> yayımlanan bütün test sayıları geliştirici kopyasında üretilmişti, yani
> temiz bir klonda **birebir tekrarlanamıyordu**. Kanonik ölçüm artık temiz
> klon; tekrar üretilebilirlik iddiasının anlamı budur.
>
...
| Testler | ✅ **3.578 test yeşil** (3.632 toplanan · 54 atlanan · **0 başarısız**), ağ gerektirmeden koşuyor — atlananların 53'ü Postgres/pgvector isteyen testlerdir (CI'ın `test-with-deps` işinde koşar), 54.'sü ise izlenmeyen bir anotatör `.xlsx` fixture'ına bağlıdır (aşağıdaki nota bakın). Ölçüm 2026-08-23: `python -m pytest tests -q` → `3578 passed, 54 skipped, 1724 subtests passed`. İki koşucu birebir aynı. Kanıt tazeliği kapısı (`scripts.kanit_tazeligi`) bu sayıyı her koşumda artefaktla karşılaştırır; sapma CI'ı kırar |
...
`gold.round1` **0,284**. Bu bir model kötüleşmesi **değildir** — paydanın
...
```
Kanıt: `eval/reports/20260823-073019/` (`gold.v2`) ve
`eval/reports/20260823-073045/` (`gold.round1`, **HAKEM-05 sonrası, temiz ağaç**).
21 Ağustos koşumu (`20260821-182450/` · `20260821-182525/`) da depoda ve aynı
sayıları veriyor — iki farklı commit, 36 alanda 0 sapma.

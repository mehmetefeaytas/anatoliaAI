---
title: "Kâr Payı Oranı"
tags: [concept, finans, terminoloji]
source: "[[2026-06-16-teknofest-tyda-sartname-2-senaryo]]"
date: 2026-06-16
status: stable
---

# Kâr Payı Oranı

[[katilim-bankaciligi]]'nda **faiz yerine** kullanılan, finansman işlemine konu
olan mal veya hizmet üzerinden oluşan kâr payı oranı (şartname 5.5, s.9).

- Çıkarılması beklenen **en kritik finansal bilgilerden** biridir; model farklı
  ifade biçimlerini doğru yorumlamalıdır: "%2,05 kâr payı oranı", "avantajlı kâr
  payı fırsatı", "özel oranlı finansman", "düşük maliyetli finansman"
  (şartname 5.2, s.7).
- Aynı değer farklı yazılabilir (%2,05 / % 2.05 / 2.05 %) → normalize edilmeli,
  bkz. [[veri-normalizasyonu]] ve [[farkli-ifade-bicimleri]].
- Karşılaştırma kriterlerinde "En Düşük Kâr Payı Oranı" yer alır (şartname 5.7,
  s.10) → [[urun-karsilastirma]].

İlgili diğer terimler: finansman maliyeti, katılım fonu, masrafsız finansman,
avantajlı finansman (şartname 5.5).

## Katılma hesabı ayağı (2026-08-24)

Bu sayfa ağırlıkla **finansman** tarafındaki kâr payı oranını anlatıyor
(murabaha kâr marjı). Katılma hesabı tarafı ayrı bir büyüklüktür ve verisi
korpusta değil TKBB'nin haftalık tablolarındadır:
[[tkbb-kar-payi-veri-seti]]. Orada **iki** oran vardır ve karıştırılmamaları
gerekir — dağıtılan kâr payı (getiri) ile kâr paylaşım oranı (bölüşüm):
[[katilma-orani-iki-ayri-buyukluk]].

## Sources
- [[2026-06-16-teknofest-tyda-sartname-2-senaryo]] — 5.2 (s.7), 5.5 (s.9),
  5.7 (s.10)

## Related
- [[katilim-bankaciligi]] — kavramın ait olduğu alan
- [[bilgi-cikarimi]] — bu değerin çıkarılması
- [[veri-normalizasyonu]] — değerin standartlaştırılması
- [[farkli-ifade-bicimleri]] — ilgili sorun
